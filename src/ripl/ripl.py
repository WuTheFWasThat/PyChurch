...
class LocalRIPL(RIPL):
    ...
    def __init__(self, state_type = "randomdb", kernel_type = "MH"):
        self.state_type = state_type
        self.kernel_type = kernel_type
        self.directives_lock = threading.RLock()
        self.continuous_inference = False
        self.infer_thread = None

        print "Constructing RIPL: " + str((state_type, kernel_type))

        if state_type == "stateless" and kernel_type == "rejection":
            # used to implement ASSUME and OBSERVE
            self.evaluator = evaluators.Stateless(self)

            # used to implement INFER only --- but calls the evaluator
            # for all of its subroutines, e.g. MH proposals, exploring
            # value settings of an XRP when doing something Gibbs-like,
            # etc.
            self.kernel = kernels.Rejection(self)
        elif state_type == "randomdb" and kernel_type == "MH":
            self.evaluator = evaluators.RandomDB(self)
            self.kernel = kernels.RandomDBMetropolisHastings(self)
        else:
            raise Exception("not implemented")

        # once we have others, figure out delegation more cleanly
        ...
        # install builtin procedures
        for (builtin_xrp_name, builtin_xrp) in xrps.ALL_XRPS.items():
            # FIXME: Thread Kernel through all XRPs, and use that to let XRPs
            #        know about multiple inference tactics
            self.global_env.bind(builtin_xrp_name, builtin_xrp(self.evaluator, ["global", builtin_xrp_name]))

            # FIXME: propagate results through
            #        mem inc/remove can use the evaluator to uneval when needed
            #        but doesn't actually need to know the application location
